...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (error === 'access_denied') {
    return res.redirect(302, '/vendors?square=denied');
  }

  if (!code || !stateRaw) {
    return res.redirect(302, '/vendors?square=error&reason=missing_params');
  }
  ...
  if (!cookieState || cookieState !== state.csrf) {
    return res.redirect(302, '/vendors?square=error&reason=csrf_mismatch');
  }
