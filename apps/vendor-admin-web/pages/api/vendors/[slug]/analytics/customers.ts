...
type CustomerSummaryResponse =
  | { success: true; data: import('@countrtop/models').CustomerSummary }
 ...
 * 
 * Returns customer summary metrics (CountrTop online orders only).
 * Note: This endpoint only includes CountrTop online orders (excludes POS orders).
 */
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<CustomerSummaryResponse>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      error: `Failed to fetch customer summary: ${errorMessage}`
    });
  }
}
