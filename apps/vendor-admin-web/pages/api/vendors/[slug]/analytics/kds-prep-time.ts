...
type KdsPrepTimeResponse =
  | { success: true; data: import('@countrtop/models').KdsPrepTimePoint[] }
  | { success: false; error: string };

/**
 * GET /api/vendors/[slug]/analytics/kds-prep-time
 * 
 * Query Parameters:
 ...
 * 
 * Returns KDS prep time time series data.
 */
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<KdsPrepTimeResponse>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    const granularity = (typeof granularityParam === 'string' &&
      (granularityParam === 'hour' || granularityParam === 'day' || granularityParam === 'week'))
      ? granularityParam
      : 'day';
    ...
    // Get KDS prep time series
    const prepTime = await dataClient.getKdsPrepTimeSeries(
    ...
    console.error('Error fetching KDS prep time:', error);
    return res.status(500).json({
      success: false,
      error: `Failed to fetch KDS prep time: ${errorMessage}`
    });
  }
}
