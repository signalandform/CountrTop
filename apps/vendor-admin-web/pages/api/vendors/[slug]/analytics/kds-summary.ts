...
/**
 * GET /api/vendors/[slug]/analytics/kds-summary
 * 
 * Query Parameters:
 ...
 * 
 * Returns KDS summary metrics for the vendor's location.
 */
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<KdsSummaryResponse>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    // Get KDS summary
    const summary = await dataClient.getKdsSummary(
    ...
    return res.status(500).json({
      success: false,
      error: `Failed to fetch KDS summary: ${errorMessage}`
    });
  }
}
