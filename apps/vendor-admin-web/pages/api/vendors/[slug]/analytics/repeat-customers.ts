...
type RepeatCustomerMetricsResponse =
  | { success: true; data: import('@countrtop/models').RepeatCustomerMetrics }
  | { success: false; error: string };

/**
 * GET /api/vendors/[slug]/analytics/repeat-customers
 * 
 * Query Parameters:
 *   - startDate (ISO 8601, required)
 *   - endDate (ISO 8601, required)
 * 
 * Returns repeat customer metrics (CountrTop online orders only).
 * Note: This endpoint only includes CountrTop online orders (excludes POS orders).
 */
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<RepeatCustomerMetricsResponse>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    // Get repeat customer metrics (CountrTop online orders only)
    const repeatCustomerMetrics = await dataClient.getRepeatCustomerMetrics(
      vendor.id,
      startDate,
      endDate
    );
    ...
    return res.status(500).json({
      success: false,
      error: `Failed to fetch repeat customer metrics: ${errorMessage}`
    });
  }
}
