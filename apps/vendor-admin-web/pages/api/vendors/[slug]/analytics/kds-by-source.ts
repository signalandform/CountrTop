...
type KdsBySourceResponse =
  | { success: true; data: import('@countrtop/models').KdsSourceMetrics }
  | { success: false; error: string };

/**
 * GET /api/vendors/[slug]/analytics/kds-by-source
 * 
 * Query Parameters:
 *   - startDate (ISO 8601, required)
 *   - endDate (ISO 8601, required)
 * 
 * Returns KDS metrics broken down by source (online vs POS).
 */
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<KdsBySourceResponse>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    // Get KDS by source metrics
    const bySource = await dataClient.getKdsBySource(
    ...
    console.error('Error fetching KDS by source:', error);
    return res.status(500).json({
      success: false,
      error: `Failed to fetch KDS by source: ${errorMessage}`
    });
  }
}
