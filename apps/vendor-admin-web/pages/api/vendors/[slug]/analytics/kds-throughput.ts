...
type KdsThroughputResponse =
  | { success: true; data: import('@countrtop/models').KdsThroughputPoint[] }
  | { success: false; error: string };

/**
 * GET /api/vendors/[slug]/analytics/kds-throughput
 * 
 * Query Parameters:
 ...
 * 
 * Returns KDS throughput time series data.
 */
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<KdsThroughputResponse>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    // Get KDS throughput
    const throughput = await dataClient.getKdsThroughput(
    ...
    console.error('Error fetching KDS throughput:', error);
    return res.status(500).json({
      success: false,
      error: `Failed to fetch KDS throughput: ${errorMessage}`
    });
  }
}
