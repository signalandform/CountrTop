...
type AovSeriesResponse =
  | { success: true; data: import('@countrtop/models').AovPoint[] }
  | { success: false; error: string };

/**
 * GET /api/vendors/[slug]/analytics/aov-series
 * 
 * Query Parameters:
 ...
 * 
 * Returns Average Order Value (AOV) time series data.
 */
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<AovSeriesResponse>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    // Get AOV series
    const aovSeries = await dataClient.getAovSeries(
    ...
    console.error('Error fetching AOV series:', error);
    return res.status(500).json({
      success: false,
      error: `Failed to fetch AOV series: ${errorMessage}`
    });
  }
}
